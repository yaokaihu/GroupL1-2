...
def main():
    ...
    parser.add_argument('--dataset', type=str, required=True,default='mnist', help='dataset (mnist|cifar10|cifar100)')
    parser.add_argument('--network', type=str, required=True,default='lenet',
                        help='network (lenet|resnet20|resnet50|vgg16')
    parser.add_argument('--penalty', type=float, required=True,default=0.5, help='regularization type(0.5(GL1/2) | 2(GL) )')
    parser.add_argument('--reg_param', type=float, required=True, default=0.002, help='regularization parameter')
    ...
    '''1. Pre Train'''
    print(
        f'{args.network}-{args.dataset}-{args.penalty}\t ʎ:{args.reg_param}\t thre:{args.thre}\t {time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())}')

    # get hyper-parameter
    network, optimizer, train_iteration, train_batch_size, test_batch_size , gamma, stepvalue = get_hyperparameters(
        args.network, args.dataset)
    # train
    train(train_dataset, network, optimizer, train_iteration, train_batch_size,
          args.penalty,
          args.reg_param, gamma, stepvalue, args.network, path=prunedPath)
    ...
    print(f'{top1_acc:.2f}, {top5_acc:.2f},{weight_sparsity:.2f},{filter_sparsity:.2f}')
